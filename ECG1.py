...
CLASS_MAPPING = {
    'N': 'N', 'L': 'N', 'R': 'N', 'B': 'N', 'e': 'N', 'j': 'N', 'A': 'S', 'a': 'S',
    'J': 'S', 'S': 'S', 'V': 'V', 'r': 'V', 'F': 'F', 'Q': 'Q', '/': 'Q', 'f': 'Q',
    '~': 'Q', '"': 'Q', '[': 'Q', ']': 'Q', 'x': 'Q', '?': 'Q', '|': 'Q', 'o': 'Q',
    '!': 'Q', '+': 'Q', '(': 'Q', ')': 'Q', 'p': 'Q', 't': 'Q', 'u': 'Q', '`': 'Q', '^': 'Q'
}

# --- Preprocessing Functions ---
def butter_bandpass(lowcut, highcut, fs, order=5):
    nyq = 0.5 * fs; low, high = lowcut / nyq, highcut / nyq
...
def segment_heartbeats_fixed_window(ecg_signal, r_peaks, segment_length, pre_r_percent=0.3, post_r_percent=0.7):
    ...
    for r_peak in r_peaks:
        ...
        r_peak_indices.append(r_peak)
    return np.array(segments), np.array(r_peak_indices)

# --- Data Loading Pipeline ---
def load_and_preprocess_mitbih_enhanced(records, sampling_rate, segment_length, class_mapping, lowcut, highcut, notch_freq, notch_Q,
                                      r_peak_detector='xqrs', segment_pre_r_percent=0.3, segment_post_r_percent=0.7):
    ...
    for rec_name in tqdm(records, desc="Processing MIT-BIH Records"):
        try:
            ...
            denoised_signal = iir_notch_filter(butter_bandpass_filter(ecg_signal, lowcut, highcut, sampling_rate), notch_freq, notch_Q, sampling_rate)
            if r_peak_detector == 'xqrs':
                xqrs = wfdb.processing.XQRS(sig=denoised_signal, fs=sampling_rate); xqrs.detect(); r_peaks = xqrs.qrs_inds
            else: # annotation
                r_peaks = np.array([s for i, s in enumerate(annotation.sample) if annotation.symbol[i] in ['N', 'L', 'R', 'B', 'A', 'a', 'J', 'S', 'V', 'r', 'F', '/', 'f']])
            r_peaks = np.unique(r_peaks); r_peaks.sort()
            segments, r_peak_indices = segment_heartbeats_fixed_window(denoised_signal, r_peaks, segment_length, segment_pre_r_percent, segment_post_r_percent)
            ...
            for r_idx in r_peak_indices:
                ...
                if np.abs(annotation.sample[closest_ann_idx] - r_idx) <= 5:
                    mapped_label = class_mapping.get(annotation.symbol[closest_ann_idx], 'Q')
                else: mapped_label = 'Q'
                labels_for_segments.append(mapped_label)
            all_segments.extend(normalized_segments); all_labels.extend(labels_for_segments)
        except Exception as e: print(f"Error processing record {rec_name}: {e}")
    ...
    for label, count in zip(unique_labels, counts): print(f"  {label}: {count} ({count/len(all_labels)*100:.2f}%)")
...
class DynamicKernelSwitching(layers.Layer):
    ...
    def call(self, inputs):
        ...
        weighted_sum = tf.add_n([conv_out * K.expand_dims(expanded_weights[:, :, i], axis=-1) for i, conv_out in enumerate(conv_outputs)])
        return weighted_sum
    def get_config(self):
...
class SelfAttention(layers.Layer):
    def __init__(self, units, **kwargs):
        ...
        self.units = units
        self.W = layers.Dense(units); self.U = layers.Dense(units); self.V = layers.Dense(1)
    def call(self, inputs):
    ...
    def get_config(self):
        ...
        config.update({'units': self.units}); return config
...
if __name__ == "__main__":
    ...
    SEGMENT_PRE_R_PERCENT, SEGMENT_POST_R_PERCENT = 0.3, 0.7
